...
dlsa<-function(X, Y, K, fit_fun, ind = NULL, ...)
{
  ...
  # get the result
  Sig_inv_list = lapply(fit_res,extrac_Sig_inv)
  Sig_inv_theta_list = lapply(fit_res,extrac_Sig_inv_theta)
  theta_mat = matrix(unlist(lapply(fit_res,extrac_theta)) ,p , K)
